...
class GenerationConfiguration:
    ...
    def __post_init__(self) -> None:
        ...
        default_template_md_options = {"badge_as_image": False, "toc_template_name": "classic"}
        default_template_md_options.update(self.template_md_options or {})
        self.template_md_options = default_template_md_options

    def get_toc_template_file_path(self) -> str:
        """
        get toc template's absolute path deduced from config toc_template_name
        raise exception if template file does not exists
        """
        template_path = os.path.join(
            self.templates_directory,
            self.template_name,
            "toc_menu_" + self.template_md_options["toc_template_name"] + ".html",
        )
        if not (Path(template_path).is_file()):
            raise Exception(f"toc_template_name, file does not exist : {template_path}")
...
class MdTemplate(object):
    def __init__(self, config, env, templates_directory):
        self.headings = {}
    ...
    def heading(
        self, title: str, depth: int, html_id: Union[bool, str] = False, node: Union[bool, SchemaNode] = False
    ) -> str:
        """
        Filter. display heading menu, heading number automatically calculated
        from previous heading and depth provided

        You can provide node paramter to be passed to the toc array
        if rendering of heading in the toc depends on a node.
        If node provided, html_id is automatically retrieved from node
        """
        html_id = False
        if node:
        ...
        for cur_depth in range(
            depth + 1, max(int(heading) for heading in self.headings) + 1 if self.headings else depth + 1
        ):
            self.headings.pop(cur_depth, None)

        # compute heading, for each depth get last level, and increment level of asked depth
        heading_numbers = ""
        for cur_depth in range(0, depth + 1):
            if cur_depth in self.headings:
                if cur_depth == depth:
                    self.headings[cur_depth] = self.headings[cur_depth] + 1
            else:
                self.headings[cur_depth] = 1
            if cur_depth != 0:
                heading_numbers += f"{self.headings[cur_depth]}."
        ...
        #  finally generate markdown title with anchor (except if depth 0)
        heading = "#" * (depth + 1)

        if depth == 0:
            heading += f" {title}"
        else:
            heading += f' <a name="{html_id}"></a>{heading_numbers} {title}'

        return heading

    def get_toc(self) -> str:
        """
        generate Table Of Content from all the headings collected
        """

        # copy toc removing first heading
        toc_copy = self.toc[1:]
        second_heading_depth = 0
        for idx, heading in enumerate(toc_copy):
            if idx == 0:
                second_heading_depth = heading["depth"]
            # Ensure we have no space at first level for TOC to be recognized by md preview
            toc_copy[idx]["indent"] = "  " * (heading["depth"] - second_heading_depth)

        # generate toc from template
        template_path = self.config.get_toc_template_file_path()

        with open(template_path, "r") as template_fp:
            template = self.env.from_string(template_fp.read())
        return template.render(toc=toc_copy, config=self.config)
...
def generate_from_schema(
    ...
    if config.template_name == "md":
        MdTemplate(config, env, templates_directory)
